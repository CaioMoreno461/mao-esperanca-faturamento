...
type DeleteHandler = (recordType: string, id: number, label: string) => Promise<void>;
...
export default function FinanceApp() {
  ...
  const deleteItem = useCallback(async (recordType: string, id: number, label: string) => {
    const confirmed = window.confirm(`Excluir ${label}? Esta ação não pode ser desfeita.`);
    if (!confirmed) return;
    await runAction("deleteRecord", { recordType, id }, "Registo excluído com sucesso.");
  }, [runAction]);
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="page-content">
          ...
          {view === "cases" && <CasesView cases={filteredCases} search={search} status={caseStatus} onSearch={setSearch} onStatus={setCaseStatus} onNewCase={() => setModal("case")} onPayment={() => setModal("payment")} onDelete={deleteItem} />}
          {view === "agenda" && <AgendaView data={data} onImport={() => fileInputRef.current?.click()} onNewCase={() => setModal("case")} onDelete={deleteItem} />}
          {view === "cash" && <CashView data={data} onPayment={() => setModal("payment")} onExpense={() => setModal("expense")} onPayout={() => setModal("payout")} onDelete={deleteItem} />}
          {view === "team" && <TeamView data={data} onProfessional={() => setModal("professional")} onLaboratory={() => setModal("laboratory")} onPayout={() => setModal("payout")} onDelete={deleteItem} />}
